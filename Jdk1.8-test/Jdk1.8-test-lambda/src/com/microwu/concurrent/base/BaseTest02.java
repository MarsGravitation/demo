...
//                wakeup(t);
     ...
     *
     * MESA 模型：
     *  并发编程的两大核心问题：互斥，即同一时刻只允许一个线程访问共享资源；另一个是同步，即线程之间
     *  如何通信、协作
     *
     *  互斥：管程和信号量关于互斥的实现完全一样，都是讲共享变量及其操作统一封装起来
     *  同步：管程在信号量的基础上，增加了条件同步，将复杂的操作封装起来
     *
     *  JUC AQS 也是基于管程实现的，基于 ReentrantLock 实现一个阻塞队列。阻塞队列有两个操作，
     *  分别是入队和出队，这两个方法都是先获取互斥锁，类比管程模型的入口。
     ...
     *      4. 如果出队成功，那么队列就不满了，就需要通知条件变量；队列不满 notFull 对应的等待队列
     *
     *  入队和出队都必须获取互斥锁
     *
     *  wait 的正确姿势：
